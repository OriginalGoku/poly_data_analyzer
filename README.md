...
- UI: `/scenario-builder` to author scenario JSONs via guided form (with per-field sweep toggles), `/scenario-runner` to launch runs, `/scenario-results` to browse outputs, `/nba-band-drop-recovery` to compute band x drop-pct conditional recovery base rates via the `band_drop_recovery_sweep` scenario
