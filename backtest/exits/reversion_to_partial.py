"""reversion_to_partial exit: first post-trigger trade at or above (anchor - partial_cents/100)."""
...
def _scan(ctx: Context, trigger: Trigger, params: Mapping[str, Any], now: datetime) -> Optional[Exit]:
    if trigger.anchor_price is None:
        return None
    partial_cents = int(params["partial_cents"])
    target = round(float(trigger.anchor_price) - partial_cents / 100.0, 4)
...
def reversion_to_partial(ctx: Context, trigger: Trigger, params: Mapping[str, Any]):
