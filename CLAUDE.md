...
- `analysis_nba_open_vs_tipoff.py` -- Export CLI. Emits `dataset.csv`, per-band summaries, plus `tipoff_band_min_price_distribution.csv` and `tipoff_band_stop_loss_ev.csv`; appends an argmax-per-band "Stop-Loss EV" section to `summary.md`. Baseline archival: `scripts/archive_baseline_pre_tipoff_stoploss.sh`.
