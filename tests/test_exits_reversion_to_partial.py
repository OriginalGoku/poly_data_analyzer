"""Tests for backtest.exits.reversion_to_partial."""
...
from backtest.exits.reversion_to_partial import reversion_to_partial
...
def _make_ctx(trades_df: pd.DataFrame, tipoff: datetime, game_end: datetime,
    ...
    scenario = Scenario(
        ...
        exit=ComponentSpec(name="reversion_to_partial", params={"partial_cents": 5}),
...
def test_partial_target(base_time):
    # anchor 0.92 - 5c = 0.87 target
    game_end = base_time + timedelta(hours=2)
    entry_t = base_time + timedelta(minutes=5)
    df = _trades([
        (entry_t, 0.81, FAV, "tok_fav"),
        (base_time + timedelta(minutes=8), 0.84, FAV, "tok_fav"),
        (base_time + timedelta(minutes=12), 0.87, FAV, "tok_fav"),  # hits target
        (base_time + timedelta(minutes=15), 0.92, FAV, "tok_fav"),
    ])
    ctx = _make_ctx(df, base_time, game_end)
    scanner = reversion_to_partial(ctx, _trigger(entry_t), {"partial_cents": 5})
    assert isinstance(scanner, ExitScanner)
    ex = scanner.scan(ctx, game_end)
    assert ex is not None
    assert ex.exit_price == 0.87
    assert ex.exit_kind == "reversion"
    assert ex.status == "filled"


def test_no_partial_revert(base_time):
    game_end = base_time + timedelta(hours=2)
    entry_t = base_time + timedelta(minutes=5)
    df = _trades([
        (entry_t, 0.81, FAV, "tok_fav"),
        (base_time + timedelta(minutes=10), 0.85, FAV, "tok_fav"),
        (base_time + timedelta(minutes=15), 0.86, FAV, "tok_fav"),
    ])
    ctx = _make_ctx(df, base_time, game_end)
    scanner = reversion_to_partial(ctx, _trigger(entry_t), {"partial_cents": 5})
...
def test_partial_zero_cents_equals_anchor(base_time):
    game_end = base_time + timedelta(hours=2)
    entry_t = base_time + timedelta(minutes=5)
    df = _trades([
        (entry_t, 0.81, FAV, "tok_fav"),
        (base_time + timedelta(minutes=10), 0.91, FAV, "tok_fav"),
        (base_time + timedelta(minutes=15), 0.92, FAV, "tok_fav"),
    ])
    ctx = _make_ctx(df, base_time, game_end)
    scanner = reversion_to_partial(ctx, _trigger(entry_t), {"partial_cents": 0})
    ex = scanner.scan(ctx, game_end)
    assert ex is not None and ex.exit_price == 0.92


def test_registry_entry():
    from backtest import exits as _  # noqa: F401
    assert EXITS["reversion_to_partial"] is reversion_to_partial
